...
Liberty Next ships with **[Nomaflow](https://docs.nomana-it.fr/liberty/nomaflow/overview/)**, a prebuilt scheduler that is far easier to deploy than Airflow — so this Airflow-based scheduler is **no longer needed**. The heavy Airflow + FastAPI + React stack documented below is replaced by an integrated, ready-to-run scheduling solution.
